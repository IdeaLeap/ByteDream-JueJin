...
<template>
  <li class="focus:text-slate-500 px-6 py-4 transition-all bg-white hover:bg-gray-50 b-b b-grey all-cursor-pointer">
    <NuxtLink :to="`/${uname}`">
      <div class="flex items-center all-px-3">
        <span class="text-4 border-r-1 pl-0">{{ uname }}</span>
      ...
      <div class="py-4">
        <div class="title font-semibold tracking-wide" style="font-size: 1.30rem;">
          {{ title }}
        </div>
        <div class="pt-4 text-slate-500 truncate" style="font-size: 1.05rem;">
          {{ desc }}
        </div>
      </div>
      <div class="flex all-flex text-base all-items-center all-text-slate-700">
        <div>
